...
**Start here:** Build 6 scenes manually → set each morph time to 20–30s → record the output → import into Ableton/Reaper/FL.
...
Switch between systems to compare their dynamical behavior sonically — you hear the difference between Lorenz and Rössler in a way that a plot doesn't capture. Adjust coupling K in Kuramoto and listen to the synchronization transition happen. The bifurcation diagram shows the period-doubling route to chaos; the audio makes it visceral.
...
| System | Character |
|--------|-----------|
| **Lorenz** | Chaotic butterfly. Sensitive to σ/ρ/β. The classic. |
| **Rössler** | Slower spiral chaos. More melodic, more patient. |
...
| Mode | What it does |
|------|-------------|
| **Direct** | State variables → oscillator frequencies quantized to a musical scale |
| **Orbital** | Angular velocity = fundamental; Lyapunov exponent → inharmonicity |
...
| **Melodic** | Glass Harp, Siren Call, Möbius Lead, Electric Kelp, The Butterfly's Aria |
...
Math Sonify has behaviors that have no UI, no settings, and no documentation beyond this paragraph. The app is aware of the time of day and sounds slightly different at 2am than at noon. It is aware of the season and shifts pitch a fraction of a percent across the calendar year. It tracks the lunar cycle and the phase portrait is marginally brighter on a full moon. The attractor's filter warms up over the first hour like a tube amp. After five minutes of no interaction with Evolve on, the wander range quietly expands — the instrument exploring because nobody is steering — and contracts the moment you touch a slider. After thirty minutes idle it briefly dreams in a different mathematical universe and comes back. Your interactions accumulate as entropy that makes the instrument more adventurous over weeks. Two copies of Math Sonify running on the same machine discover each other through a local socket and faintly influence each other's behavior. The attractor resumes from where it left off every time you reopen the app, as though it was running the whole time you were away. None of this is visible. Some of it you will eventually notice and not be able to explain. That's intentional.
...
**📸 Save Clip** in the MIXER tab captures the last 60 seconds as a 32-bit float stereo WAV plus a 512×512 phase portrait PNG. Both timestamped, saved to `clips/` next to the exe.
...
Sonification::map()                                                 LayerSynth[0..2]: 4 osc + ADSR + KS
                                                                    Σ → master FX chain → limiter
UI thread (30 Hz)
─────────────────
egui: Phase portrait, MIXER, ARRANGE, Waveform, Math View, Bifurcation
```

Lock-free audio callback. All sim→audio via `try_send` (drops on backpressure). Clip buffer via `try_lock` (skips on contention).
